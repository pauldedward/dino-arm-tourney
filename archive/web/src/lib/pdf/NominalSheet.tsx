...
export function NominalSheet({ event, rows }: { event: { name: string; starts_at: string }; rows: NominalRow[] }) {
    ...
    <Document>
      <Page size="A4" style={sharedStyles.page}>
        ...
        <Text style={sharedStyles.meta}>
          Generated {new Date().toLocaleString()} · {rows.length} athletes
        </Text>
        <View style={sharedStyles.table}>
          <View style={sharedStyles.thead}>
            ...
            <Text style={[sharedStyles.th, { width: 70 }]}>District/Team</Text>
            <Text style={[sharedStyles.th, { width: 36 }]}>Wt</Text>
            <Text style={[sharedStyles.th, { width: 80 }]}>Age cats</Text>
            <Text style={[sharedStyles.th, { width: 50 }]}>Status</Text>
          </View>
          {sorted.map((r, i) => (
            <View key={i} style={sharedStyles.tr} wrap={false}>
              ...
              <Text style={[sharedStyles.td, { width: 70 }]}>{r.district ?? r.team ?? ""}</Text>
              <Text style={[sharedStyles.td, { width: 36 }]}>{r.declared_weight_kg}</Text>
              <Text style={[sharedStyles.td, { width: 80 }]}>{(r.age_categories ?? []).join(", ")}</Text>
              <Text style={[sharedStyles.td, { width: 50 }]}>{r.status}</Text>
            </View>
          ))}
        </View>
